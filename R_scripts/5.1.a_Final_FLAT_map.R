...
type_model="FLAT" # modeling strategy (FLAT or HIERARCHICAL)
...
for (l in seq(1:maxTypoLevel)) {
    ...
    for (raster_file in raster_files) {
      ...
      raster_final <- terra::rast(raster_file)
      names(raster_final) <- c("G", "NIR","GRVI","Greenness","BSI","NDWI","DTM")
      ...
      resolution <- res(raster_pred_crisp) # resolution of the raster
      radius <- 10 # radius of the smoothing window (in map units)
      window_size <- radius / resolution # window size in pixels
      ...
      smoothed_raster_pred_crisp <- focal(raster_pred_crisp, w = matrix(1, nrow = window_size[1]*2+1, ncol = window_size[2]*2+1), fun = modal) # apply smooth filter
      rm(raster_pred_crisp) # free memory
      
      # Save smooth map (.tif)
      NOMtiff3=paste0(MapFolder,"/","Smoothed_final_map_RF_",type_model,"_model_", District, "_", 
                      Island, "_", Satellite1, "_", Year,"_", Res1, "_level_", l, ".TIF")
      writeRaster(smoothed_raster_pred_crisp, NOMtiff3, overwrite = TRUE)
      
      # Convert raster to vector
      vector_map <- terra::as.polygons(smoothed_raster_pred_crisp, fun = function(x) { x > 0 })
      vector_layer <- terra::project(vector_map, "EPSG:32739")
      rm(smoothed_raster_pred_crisp)
      ...
      # Save shapefile
      FILE3=paste0(MapFolder,"/","Smoothed_simplified_final_map_RF_",type_model,"_model_", District, "_", Island, "_", Satellite1, "_", Year,"_", Res1, "_level_", l, ".shp")
