...
for (l in seq(1:maxTypoLevel)) {
    ...
    for (raster_file in raster_files) {
      ...
      resolution <- res(raster_pred) # resolution of the raster
      ...
      smoothed_raster_pred <- focal(raster_pred, w = matrix(1, nrow = window_size[1]*2+1, ncol = window_size[2]*2+1), fun = modal) # apply smooth filter
      rm(raster_pred) # free memory
      
      # Save smooth map (.tif)
      NOMtiff2=paste0(MapFolder,"/","Smoothed_final_map_RF_",type_model,"_model_", District, "_", 
                      Island, "_", Satellite1, "_", Year,"_", Res1, "_level_", l, ".TIF")
      writeRaster(smoothed_raster_pred, NOMtiff2, overwrite = TRUE)
      
      # Convert raster to vector
      vector_map <- terra::as.polygons(smoothed_raster_pred, fun = function(x) { x > 0 })
      vector_layer <- terra::project(vector_map, "EPSG:32739")
      rm(smoothed_raster_pred)
