...
# Compute GLCM features for available rasters ------------------------------------------------------------
...
for (Year1 in all_years){
  ...
  for (Month1 in all_months) {
    ...
    # define raster path
    raster_path<- file.path(open_cut_raster_path, paste0(District,"_",Island,"_",Satellite1, "_",Year1,"_",Month1,"_",Res1, "_",selected_raster,"_cut.TIF"))
