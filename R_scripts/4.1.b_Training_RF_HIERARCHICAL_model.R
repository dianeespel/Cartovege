# Script name: Training RF model
# Author: Diane ESPEL
# Objective: Calibration of RF model
...
type_model="HIERARCHICAL" # modelling strategy 
...
for (l in seq(1:maxTypoLevel)) {
  ...
  if (l == 1){ 
    # For level 1, select only plot ID, coordinates, current habitat level, and imagery features
    selected_learning_data = learning_data[, c(iid, ix, iy, ihab, ibegin:iend)]
  } else{
    # Retrieve indices of all habitat columns for levels below current 'l' (from 1 to l-1)
    ihabsup = unlist(lapply(1:(l-1), function(lsup) which(colnames(learning_data) == paste0("Hab_L", lsup))))
    
    # Select columns: Plot ID, Coordinates (X and Y), Current habitat level, All lower habitat levels, imagery feature columns from 'G' to 'Dtm'
    selected_learning_data = learning_data[, c(iid, ix, iy, ihab, ihabsup, ibegin:iend)]
  } 
